...
$order_codx = $_GET['order_codx'];
$codx = $_GET['codx'];

$sql = "SELECT * FROM m_order WHERE codx = :order_codx";
$stmt = $conn->prepare($sql);
$stmt->bindParam(':order_codx', $order_codx, PDO::PARAM_STR);
$stmt->execute();
...
$sqlb = "SELECT * FROM m_kas WHERE codx = :codx";
$stmtb = $conn->prepare($sqlb);
$stmtb->bindParam(':codx', $codx, PDO::PARAM_STR);
$stmtb->execute();
    ...
    <table width="100%">
      <tr>
        <td>
          ...
          <h4>
              <?php
                $created_at = $rowb['created_at']; // contoh: "2025-10-05 14:30:00"
    ...
    <table>
      ...
      <?php
       $count = 1;
        ...
        $sql = $conn->prepare("SELECT * FROM `m_kas` WHERE codx = :codx ORDER BY id DESC");
        $sql->bindParam(':codx', $codx, PDO::PARAM_STR); // asumsi $ocodx adalah string
        $sql->execute();
